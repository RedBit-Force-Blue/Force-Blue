...
export const RegisterPage = () => {
  ...
  const register = async (e) => {
    try {
      ...
      if (data) {
        const a = await axios.put(`http://localhost:3022/user/uploadImage/${data.id}`, img, {
          headers: {
            'Content-Type': 'multipart/form-data',
            'Authorization': data.token
          }
        })
        console.log(a.data.UpdateUserImage, 'imagen se actualiza')
        // data.logged.t4Ca59q = a.data.name;
        // await axios.post(`/api/account/create/${data.id}`, {}, {
        //   headers: {
