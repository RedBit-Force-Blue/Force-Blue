...
// constante para restringer parametros para usuarios
const restrictedInfo = "-password -role -email  -phone -email -_id";
...
// funcion para que se registre el usuario(cliente)
...
exports.login = async(req, res) => {
    try {
        ...
        let validateCredentials = verify(credentials);
        ...
        if (!user) {
        ...
        } else if (user && (await check(data.password, user.password))) {
            ...
            return res.send({ message: `Welcome to SCN PROJECT ${user.name}`, token, logged });
        } else {
        ...
        return res.status(400).send({
            message: "error accessing your session, please check your credentials or try later",
        });
    }
};
...
exports.UpdateUser = async(req, res) => {
    try {
        ...
        if (userId.sub == paramsId) {
            ...
            let UpdatingUser = await User.findOneAndUpdate({ _id: userId.sub },
                data, { new: true, projection: { _id: 0, password: 0, role: 0 } }
            );
...
exports.addImage = async(req, res) => {
    try {
        ...
        if (
            ...
            const updateUserImage = await User.findOneAndUpdate({ _id: userId }, { image: fileName }, { new: true })
